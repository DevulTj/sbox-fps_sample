...
public partial class PlayerController
{
	...
	private Rotation _lastInputRotation = Rotation.Identity;
	...
	private void SimulateRotation()
	{
		if ( IsDefaultGravity() )
		{
			Rotation = Player.LookInput.WithPitch( 0f ).ToRotation();
		}
		else
		{
			var upDirection = -Player.Gravity;
			...
			var deltaInputRotation = Rotation.Difference(_lastInputRotation,  inputRotation);
			Rotation *= Rotation.FromYaw(deltaInputRotation.Yaw());
			_lastInputRotation = inputRotation;
		}
	}
	
	private bool IsDefaultGravity()
	{
		return Player.Gravity is { x: 0, y: 0, z: <= 0 };
	}
